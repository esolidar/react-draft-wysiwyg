...
class WysiwygEditor extends Component {
  ...
  keyBindingFn = event => {
    ...
    //   return "Backspace"; // name this whatever you want
    // }
  ...
  handleBeforeInput = () => {
    ...
    if (maxLength) {
      if (
        getCurrentTextSelection(editorState).length !== 0 &&
        (getCurrentTextSelection(editorState).length === maxLength ||
          getCurrentTextSelection(editorState).length < maxLength)
      ) {
              ...
              {toolbar.options.map((opt, index) => {
                ...
                return (
                  <Control
                    ...
                    {...controlProps}
                    config={config}
                    readOnly={this.editorProps.readOnly}
                  />
                );
              })}
              {toolbarCustomButtons &&
